...
from keystoneclient.contrib.auth.v3 import federation
...
class _BaseSAMLPlugin(federation.FederatedAuthPlugin):

    HTTP_MOVED_TEMPORARILY = 302

    # NOTE(marek-denis): DEFAULT_PROTOCOL is a value kept for backwards
    # compatibility and will be deprecated.
    DEFAULT_PROTOCOL = 'saml2'

    def __init__(self, auth_url,
                 identity_provider, identity_provider_url,
                 username, password, protocol=None,
                 **kwargs):
        ...
        """
        protocol = _BaseSAMLPlugin._resolve_protocol(protocol)
        super(_BaseSAMLPlugin, self).__init__(
            auth_url=auth_url, identity_provider=identity_provider,
            identity_provider_url=identity_provider_url, protocol=protocol,
            **kwargs)
        self.username = username
        self.password = password

    @classmethod
    def _resolve_protocol(cls, protocol):
        if protocol is None:
            LOG.warning(_LW("Default values for federated protocol are "
                            "deprecated. A protocol parameter is required"))
...
class Saml2Token(_BaseSAMLPlugin):
    ...
    def __init__(self, auth_url,
        ...
        :param protocol: Protocol to be used for the authentication.
                         ...
                         Typical value  would be: saml2
        :type protocol: string
        :param protocol: Protocol to be used for the authentication. The
                         name must be equal to one configured at the keystone
                         sp side. This value is used for building dynamic
                         authentication URL.  Typical value  would be: saml2
        :type protocol: string

        """
        super(Saml2Token, self).__init__(
            auth_url=auth_url, identity_provider=identity_provider,
...
class ADFSToken(_BaseSAMLPlugin):
    ...
    @classmethod
    def get_options(cls):
        options = super(ADFSToken, cls).get_options()
