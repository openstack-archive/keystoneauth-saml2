...
class _BaseSAMLPlugin(federation.FederationBaseAuth):
    ...
    def get_options(cls):
        ...
        options.extend([
            cfg.StrOpt('identity-provider-url', dest='identity_provider_url',
...
class Saml2Token(_BaseSAMLPlugin):
    ...
    def get_unscoped_auth_ref(self, session):
        ...
        * HTTP POST request again to the Service Provider. The body of the
        ...
        :returns: AccessInfo
        :rtype: :py:class:`keystoneauth.access.AccessInfo`
        """
...
class ADFSToken(_BaseSAMLPlugin):
    ...
    def _cookies(self, session):
        ...
        keystoneauth.session.Session object doesn't have a cookies attribute.
    ...
    def _token_dates(self, fmt='%Y-%m-%dT%H:%M:%S.%fZ'):
        ...
        should not be live too long, as currently ``keystoneauth``
        doesn't have mechanisms for reusing such tokens (every time ADFS authn
        method is called, keystoneauth will login with the ADFS instance).
    ...
    def _get_adfs_security_token(self, session):
        ...
        :raises keystoneauth.exceptions.AuthorizationFailure: when HTTP
                 response from the ADFS server is not a valid XML ADFS security
                 token.
        :raises keystoneauth.exceptions.InternalServerError: If response
    ...
    def _access_service_provider(self, session):
        ...
        :raises keystoneauth.exceptions.AuthorizationFailure: in case session
